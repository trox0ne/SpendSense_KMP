...
import ru.multiplatform.spend_sense.root.model.BottomBarItem
...
fun BoxScope.RootBottomBar(
    ...
    Row(
        ...
        BottomBarItem.getItems().forEach { item ->
            BottomBarItemView(item, item.appTab == selectedItem) {
...
fun RowScope.BottomBarItemView(
    ...
    val foreground = if (isSelected) AppThemeProvider.colors.accent
    else AppThemeProvider.colors.onSurface
    Column(modifier = Modifier.weight(1f)
        .padding(4.dp)
        .clickable { clickOnTab() },
        horizontalAlignment = Alignment.CenterHorizontally
    ) {
        Text(stringResource(bottomBarItem.title), color = foreground)
        Image(
