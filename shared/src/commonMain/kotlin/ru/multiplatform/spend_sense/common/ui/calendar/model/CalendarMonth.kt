...
data class CalendarMonth(
    ...
    private fun insertStartStubs(firstDayIsMonday: Boolean, accumulateWeek: MutableList<CalendarDay>) {
        val startStubCount = if (firstDayIsMonday) firstDayOfMonth.ordinal
        else when (firstDayOfMonth) {
            DayOfWeek.SUNDAY -> 0
            else -> firstDayOfMonth.ordinal + 1
        }

        repeat(startStubCount) {
    ...
    private fun inflateCalendar(
        ...
        repeat(numberDays) { numberDay ->
            ...
            val isStartOfWeek = if (firstDayIsMonday) dayOfWeek == DayOfWeek.MONDAY
